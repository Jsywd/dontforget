...
router.post('/interests', isAuthenticated, async (req, res) => {
  ...
  try {
    await db.query('DELETE FROM user_interests WHERE userID = ?', [req.user.userID]);
    for (const id of categoryIDs) {
      await db.query('INSERT IGNORE INTO user_interests (userID, categoryID) VALUES (?, ?)', [req.user.userID, id]);
    }
