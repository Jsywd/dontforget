...
router.post("/", isAuthenticated, async (req, res) => {
  ...
  if (!checklistID || !itemText?.trim()) {
    return res
      .status(400)
      .json({ success: false, message: "กรุณาระบุ checklistID และ itemText" });
  }
  try {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (itemText !== undefined) {
        sql += " itemText=?,";
        params.push(itemText);
      }
      if (isChecked !== undefined) {
        sql += " isChecked=?,";
        params.push(isChecked == "1" || isChecked === true ? 1 : 0);
      }
...
router.delete("/:id", isAuthenticated, async (req, res) => {
  try {
    ...
    await db.query("DELETE FROM checklist_items WHERE itemID = ?", [
      req.params.id,
    ]);
    res.json({ success: true });
  } catch (err) {
