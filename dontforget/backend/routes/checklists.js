...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (!cl.length)
      return res
        .status(404)
        .json({ success: false, message: "ไม่พบ Checklist" });

    const checklist = cl[0];
    const isOwner =
      req.isAuthenticated() && req.user.userID === checklist.userID;

    if (!checklist.isPublic && !isOwner) {
      return res
        .status(403)
        .json({ success: false, message: "Checklist นี้เป็นส่วนตัว" });
    }
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!title)
      return res
        .status(400)
        .json({ success: false, message: "กรุณากรอกชื่อ Checklist" });

    // req.file.path URL ของ Cloudinary 
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!cl.length || cl[0].userID !== req.user.userID)
        return res
          .status(403)
          .json({ success: false, message: "ไม่มีสิทธิ์แก้ไข" });

      let sql =
        "UPDATE checklists SET title=?, description=?, categoryID=?, isPublic=?";
      const params = [
        title,
        description || null,
        categoryID || null,
...
router.post("/:id/copy", isAuthenticated, async (req, res) => {
  try {
    const [cl] = await db.query(
      "SELECT * FROM checklists WHERE checklistID = ? AND isPublic = 1",
      [req.params.id],
    );
    if (!cl.length)
      return res
    ...
    const [items] = await db.query(
      "SELECT * FROM checklist_items WHERE checklistID = ?",
      [req.params.id],
    );
    for (const item of items) {
      await db.query(
        "INSERT INTO checklist_items (checklistID, itemText, sortOrder) VALUES (?, ?, ?)",
        [newCL.insertId, item.itemText, item.sortOrder],
      );
    }
    await db.query(
      "UPDATE checklists SET copyCount = copyCount + 1 WHERE checklistID = ?",
      [req.params.id],
    );
    await db.query(
      "INSERT INTO checklist_copies (originalChecklistID, copiedByUserID) VALUES (?, ?)",
...
// ดึงรายการรีพอร์ตทั้งหมดที่สถานะเป็น pending
...
router.put("/admin/reports/:id", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "ลบรายการแจ้งรีพอร์ตออกแล้ว (โพสต์ยังอยู่)",
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: "Server Error" });
