...
passport.use(new LocalStrategy({ usernameField: 'email' }, async (email, password, done) => {
  try {
    ...
    if (!rows.length) return done(null, false, { message: 'ไม่พบอีเมลนี้ในระบบ' });

    const user = rows[0];
    if (!user.password) return done(null, false, { message: 'กรุณาเข้าสู่ระบบด้วย Google' });
...
passport.use(new GoogleStrategy({
  ...
  try {
    ...
    let [rows] = await db.query('SELECT * FROM users WHERE googleID = ? OR email = ?', [profile.id, email]);

    if (rows.length) {
      // Update googleID if missing
      if (!rows[0].googleID) {
        await db.query('UPDATE users SET googleID = ?, avatar = ? WHERE userID = ?', [profile.id, avatar, rows[0].userID]);
      }
